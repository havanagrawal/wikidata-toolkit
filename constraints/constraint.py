...
from model import television as tv
...
class Constraint:
    ...
    def __init__(
        self,
        validator: Callable[[tv.BaseType], bool],
        fixer: Callable[[tv.BaseType], Iterable] = None,
        name=None,
    ):
...
def has_property(prop: wp.WikidataProperty) -> Constraint:
    ...
    def check(item: tv.BaseType) -> bool:
...
def inherits_property(prop: wp.WikidataProperty) -> Constraint:
    ...
    @item_has_parent
    def fix(item: tv.BaseType) -> Iterable:
...
def season_has_no_of_episodes_as_count_of_parts() -> Constraint:
    def check(item: tv.BaseType) -> bool:
        return (
    ...
    return Constraint(check, name=f"season_has_no_of_episodes_as_count_of_parts()")


def season_has_parts() -> Constraint:
    def check(item: tv.BaseType) -> bool:
    ...
    return Constraint(check, fixer=fix, name=f"season_has_parts()")


def has_title() -> Constraint:
    ...
    return Constraint(check, fixer=fix, name="has_title()")


def has_english_label() -> Constraint:
    ...
    def fix(item: tv.BaseType) -> Iterable:
        ...
        if label is not None:
            item.itempage.editLabels({
                "en": item.title
            })
        return []

    return Constraint(check, fixer=fix, name="has_english_label()")
